---
type: reference
status: active
created: 2026-04-15
updated: 2026-04-15
owner: AI 3
source_of_truth: false
---

# Decisions Index

> Part of [[Home]] > Decisions | See also: [[Architecture/Decision-Log|Decision Log]], [[Tasks/Platform-Sender-Pivot-Decision|Platform Sender Pivot Decision]], [[00-System/Obsidian-Knowledge-System|Obsidian Knowledge System]]

## What Belongs Here

Permanent records of product, architecture, compliance, process, and tradeoff decisions.

## Current Migration Note

The main canonical decision register is still [[Architecture/Decision-Log|Decision Log]] in `docs/Architecture/`.

## Canonical Notes

- [[Architecture/Decision-Log|Decision Log]]
- [[Tasks/Platform-Sender-Pivot-Decision|Platform Sender Pivot Decision]]

## Owner Lane

- AI 3 for structure
- relevant owning lane for decision content
