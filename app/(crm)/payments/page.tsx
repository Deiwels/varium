    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(8px)' }}
      ...
      <div style={{ width: 'min(520px,96vw)', borderRadius: 20, border: '1px solid rgba(255,255,255,.12)', background: 'linear-gradient(180deg,rgba(20,20,30,.90),rgba(10,10,20,.88))', backdropFilter: 'blur(24px)', padding: 18, color: '#e9e9e9', fontFamily: 'Inter,sans-serif', boxShadow: '0 20px 60px rgba(0,0,0,.6)' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
          {presets.map(p => (
            <button key={p.label} onClick={() => { onChange(p.f, p.t); onClose() }}
              style={{ height: 32, padding: '0 12px', borderRadius: 999, border: `1px solid ${selFrom===p.f && selTo===p.t ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.10)'}`, background: selFrom===p.f && selTo===p.t ? 'rgba(139,154,255,.18)' : 'rgba(255,255,255,.04)', color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: 11, fontFamily: 'inherit' }}>
              {p.label}
            </button>
          ))}
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 11, fontWeight: 700, color: step==='from' ? '#8b9aff' : '#8ff0b1' }}>{step==='from' ? 'Pick start →' : '← Pick end'}</div>
        </div>
          ...
          {days.map(iso => {
            ...
            return (
              <button key={iso} onClick={() => pick(iso)}
                style={{ height: 38, borderRadius: 10, border: `1px solid ${isFrom ? 'rgba(139,154,255,.75)' : isTo ? 'rgba(143,240,177,.65)' : iso===todayStr ? 'rgba(255,207,63,.50)' : 'rgba(255,255,255,.08)'}`, background: isFrom ? 'rgba(139,154,255,.20)' : isTo ? 'rgba(143,240,177,.15)' : inRange ? 'rgba(139,154,255,.08)' : 'rgba(0,0,0,.18)', color: isTo ? '#c9ffe1' : '#fff', cursor: 'pointer', opacity: inMonth ? 1 : 0.3, fontWeight: isFrom||isTo ? 900 : 500, fontSize: 12, fontFamily: 'inherit' }}>
  ...
  return (
    <Shell page="payments">
      <style>{`
        ...
        .pay-row.sel td{background:rgba(139,154,255,.07)!important}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 5, alignItems: 'center', flexShrink: 0 }}>
              ...
              <button onClick={exportCSV}
                style={{ height: 34, padding: '0 10px', borderRadius: 999, border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.08)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 800, fontSize: 10, fontFamily: 'inherit' }}>
                CSV
              </button>
              {isOwner && (
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
                            ...
                            <div style={{ minWidth: 0 }}>
                              <div style={{ fontWeight: 900, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6 }}>
                                ...
                                {isTerminal && <span style={{ fontSize: 9, letterSpacing: '.06em', textTransform: 'uppercase', padding: '2px 6px', borderRadius: 999, border: '1px solid rgba(139,154,255,.40)', background: 'rgba(139,154,255,.10)', color: '#c8d0ff' }}>Terminal</span>}
                              </div>
