...
interface WorkspaceMember { id: string; name: string; email: string; role: string; joined_at?: string }
...
function Toggle({ checked, onChange, label, sub }: { checked: boolean; onChange: (v: boolean) => void; label: string; sub?: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.14)' }}>
      ...
      <button onClick={() => onChange(!checked)} style={{ width: 44, height: 26, borderRadius: 999, border: 'none', background: checked ? 'rgba(139,154,255,.65)' : 'rgba(255,255,255,.14)', cursor: 'pointer', position: 'relative', flexShrink: 0, transition: 'background .2s' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        {uploading && progress > 0 && (
          <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,.08)', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${progress}%`, background: 'rgba(139,154,255,.65)', borderRadius: 2, transition: 'width .3s' }} />
          </div>
        )}
...
function ScheduleEditor({ userId, userName, currentSchedule, onSaved }: { userId: string; userName: string; currentSchedule: any[] | null; onSaved: () => void }) {
  ...
  return (
    <div style={{ marginTop: 8, padding: '12px 14px', borderRadius: 14, border: '1px solid rgba(139,154,255,.25)', background: 'rgba(139,154,255,.06)' }}>
      <div style={{ fontSize: 11, letterSpacing: '.10em', textTransform: 'uppercase', color: 'rgba(139,154,255,.70)', marginBottom: 8, fontWeight: 900 }}>Schedule — {userName}</div>
        ...
        {DAY_NAMES.map((day, i) => (
          <div key={i} style={{ textAlign: 'center' }}>
            ...
            <button onClick={() => { const n = [...sched]; n[i] = { ...n[i], enabled: !n[i].enabled }; setSched(n) }}
              style={{ width: '100%', height: 24, borderRadius: 6, border: `1px solid ${sched[i].enabled ? 'rgba(139,154,255,.50)' : 'rgba(255,255,255,.12)'}`, background: sched[i].enabled ? 'rgba(139,154,255,.15)' : 'rgba(255,255,255,.04)', color: sched[i].enabled ? '#c8d0ff' : 'rgba(255,255,255,.35)', cursor: 'pointer', fontSize: 9, fontWeight: 900, fontFamily: 'inherit' }}>
      ...
      <button onClick={save} disabled={saving}
        style={{ marginTop: 8, width: '100%', height: 34, borderRadius: 10, border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.12)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 12, fontFamily: 'inherit', opacity: saving ? .5 : 1 }}>
...
function UsersTab() {
  ...
  const roleColors: Record<string, { border: string; color: string }> = {
    ...
    barber: { border: 'rgba(139,154,255,.35)', color: '#c8d0ff' },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <SectionCard title="Create account">
        ...
        <button onClick={createUser} disabled={creating}
          style={{ height: 44, borderRadius: 12, border: '1px solid rgba(139,154,255,.65)', background: 'rgba(139,154,255,.14)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', opacity: creating ? .5 : 1 }}>
...
function WorkspaceTab() {
  ...
  const [inviteRole, setInviteRole] = useState<'admin'|'barber'|'student'>('barber')
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const ws = await apiFetch('/api/workspaces/current')
      setWorkspaceName(ws?.name || '')
      setWorkspaceId(ws?.id || '')
      setMembers(ws?.members || [])
      setPendingInvites(ws?.pending_invites || [])
    } catch (e: any) {
      // fallback: try reading from user data
      try {
        const user = JSON.parse(localStorage.getItem('VURIUMBOOK_USER') || '{}')
        setWorkspaceName(user.workspace_name || 'My Workspace')
  ...
  async function sendInvite() {
    ...
    if (!workspaceId) { setInviteMsg('Workspace not loaded'); return }
    setSendingInvite(true); setInviteMsg('')
    try {
      await apiFetch(`/api/workspaces/${encodeURIComponent(workspaceId)}/invite`, {
        method: 'POST',
  ...
  async function revokeInvite(inviteId: string) {
    ...
    try {
      await apiFetch(`/api/workspaces/${encodeURIComponent(workspaceId)}/invite/${encodeURIComponent(inviteId)}`, { method: 'DELETE' })
      load()
    } catch (e: any) { alert(e.message) }
  }

  const memberRoleColors: Record<string, { border: string; color: string }> = {
    owner: { border: 'rgba(255,207,63,.35)', color: '#ffe9a3' },
    admin: { border: 'rgba(143,240,177,.35)', color: '#c9ffe1' },
    barber: { border: 'rgba(139,154,255,.35)', color: '#c8d0ff' },
    student: { border: 'rgba(168,107,255,.35)', color: '#d4b8ff' },
  ...
  if (loading) return <div style={{ padding: 40, textAlign: 'center', color: 'rgba(255,255,255,.40)' }}>Loading workspace…</div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 680 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.14)' }}>
          {editingName ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
              <input value={nameInput} onChange={e => setNameInput(e.target.value)} placeholder="Workspace name" style={{ ...inp, flex: 1 }}
                onKeyDown={e => { if (e.key === 'Enter') saveName(); if (e.key === 'Escape') setEditingName(false) }} autoFocus />
              <SmBtn onClick={saveName} disabled={savingName}>{savingName ? 'Saving…' : 'Save'}</SmBtn>
              <SmBtn onClick={() => setEditingName(false)}>Cancel</SmBtn>
            </div>
          ) : (
            <>
              <div>
                <div style={{ fontSize: 16, fontWeight: 900, letterSpacing: '.04em' }}>{workspaceName || 'Unnamed Workspace'}</div>
                {workspaceId && <div style={{ fontSize: 11, color: 'rgba(255,255,255,.30)', marginTop: 2 }}>ID: {workspaceId}</div>}
              </div>
              <SmBtn onClick={() => { setNameInput(workspaceName); setEditingName(true) }}>Edit</SmBtn>
            </>
          )}
        </div>
      </SectionCard>
      ...
      {/* Invite Team Members */}
      <SectionCard title="Invite team members">
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,.40)', marginBottom: 4 }}>Send an invite link via email to add someone to your workspace</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            <Field label="Email address">
              <input value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} placeholder="name@example.com" type="email" style={inp}
          ...
          <button onClick={sendInvite} disabled={sendingInvite}
            style={{ height: 42, padding: '0 20px', borderRadius: 12, border: '1px solid rgba(139,154,255,.65)', background: 'rgba(139,154,255,.14)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', whiteSpace: 'nowrap', opacity: sendingInvite ? .5 : 1 }}>
            {sendingInvite ? 'Sending…' : 'Send Invite'}
          </button>
        </div>
        {inviteMsg && <div style={{ fontSize: 12, color: inviteMsg.includes('Error') ? '#ffd0d0' : '#c9ffe1', padding: '8px 12px', borderRadius: 10, border: `1px solid ${inviteMsg.includes('Error') ? 'rgba(255,107,107,.30)' : 'rgba(143,240,177,.30)'}`, background: inviteMsg.includes('Error') ? 'rgba(255,107,107,.08)' : 'rgba(143,240,177,.08)' }}>{inviteMsg}</div>}

        {/* Pending Invites */}
        {pendingInvites.length > 0 && (
          <div style={{ marginTop: 6 }}>
            <div style={{ ...lbl, marginBottom: 8 }}>Pending invites ({pendingInvites.length})</div>
            {pendingInvites.map(inv => {
              const rc = memberRoleColors[inv.role] || memberRoleColors.barber
              return (
                <div key={inv.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(255,207,63,.15)', background: 'rgba(255,207,63,.04)', marginBottom: 6 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase', padding: '3px 9px', borderRadius: 999, border: `1px solid ${rc.border}`, background: 'rgba(0,0,0,.14)', color: rc.color }}>{inv.role}</span>
                  <SmBtn danger onClick={() => revokeInvite(inv.id)}>Revoke</SmBtn>
                </div>
              )
            })}
          </div>
      ...
      {/* Team Members */}
      <SectionCard title={`Team members (${members.length})`} action={<SmBtn onClick={load}>↻</SmBtn>}>
        {members.length === 0 ? (
          <div style={{ color: 'rgba(255,255,255,.30)', fontSize: 12, padding: '8px 0' }}>No team members yet — send an invite above</div>
        ) : (
          members.map(m => {
            const rc = memberRoleColors[m.role] || memberRoleColors.barber
            return (
              <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.14)' }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 900, fontSize: 14 }}>{m.name || m.email}</div>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,.40)', marginTop: 2 }}>
                    {m.email}{m.joined_at ? ` · Joined ${new Date(m.joined_at).toLocaleDateString()}` : ''}
                  </div>
                </div>
                <span style={{ fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase', padding: '3px 9px', borderRadius: 999, border: `1px solid ${rc.border}`, background: 'rgba(0,0,0,.14)', color: rc.color }}>{m.role}</span>
              </div>
            )
          })
        )}
...
export default function SettingsPage() {
  const [tab, setTab] = useState<'workspace'|'shop'|'fees'|'booking'|'payroll'|'square'|'users'>('workspace')
    ...
    <Shell page="settings">
        ...
        <div style={{ padding: '12px 20px', background: 'rgba(0,0,0,.80)', backdropFilter: 'blur(14px)', borderBottom: '1px solid rgba(255,255,255,.08)', position: 'sticky', top: 0, zIndex: 20, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <button onClick={save} disabled={saving || loading}
              style={{ height: 40, padding: '0 20px', borderRadius: 999, border: '1px solid rgba(139,154,255,.75)', background: 'rgba(0,0,0,.75)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', boxShadow: '0 0 18px rgba(139,154,255,.25)', opacity: saving ? .5 : 1 }}>
            ...
            {tab === 'shop' && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 14 }}>
                <SectionCard title="Shop info">
                  <Field label="Shop name"><input value={s.shop_name || ''} onChange={e => set('shop_name', e.target.value)} placeholder="VuriumBook Shop" style={inp} /></Field>
            ...
            {tab === 'fees' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <SectionCard title="Custom charges & categories"
                  action={<SmBtn onClick={() => { setCharges(c => [...c, { id: 'charge_'+Date.now(), name: '', type: 'percent', value: 0, color: '#8b9aff', enabled: true }]); setDirty(true) }}>+ Add</SmBtn>}>
                  ...
                  {charges.length === 0 ? <div style={{ color: 'rgba(255,255,255,.30)', fontSize: 12 }}>No custom charges</div> :
                    charges.map((c, i) => (
                      <div key={c.id} style={{ display: 'grid', gridTemplateColumns: '1fr 100px 100px 80px 36px', gap: 8, alignItems: 'center', padding: '10px 12px', borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.14)' }}>
                        ...
                        <input type="color" value={c.color||'#8b9aff'} onChange={e => { const n=[...charges]; n[i]={...n[i],color:e.target.value}; setCharges(n); setDirty(true) }} style={{ height: 34, width: '100%', borderRadius: 8, border: '1px solid rgba(255,255,255,.10)', background: 'none', cursor: 'pointer', padding: 2 }} />
                    ...
                    <div style={{ gridColumn: '1 / -1' }}>
                      ...
                      <div style={{ marginTop: 10, padding: '10px 14px', borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.14)', fontSize: 12, color: 'rgba(255,255,255,.55)' }}>
                        ...
                        {(payroll.tip_options || [15,20,25]).map((p: number, i: number) => (
                          <span key={i} style={{ marginRight: 8, padding: '2px 10px', borderRadius: 999, border: '1px solid rgba(139,154,255,.40)', background: 'rgba(139,154,255,.10)', color: '#c8d0ff', fontSize: 11 }}>{p}%</span>
                        ))}
                  ...
                  {squareOAuth.connected ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 14px', borderRadius: 12, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.14)' }}>
                      ...
                      <button onClick={connectSquare} disabled={squareConnecting}
                        style={{ height: 38, padding: '0 20px', borderRadius: 999, border: 'none', background: 'rgba(139,154,255,.75)', color: '#fff', cursor: 'pointer', fontWeight: 700, fontSize: 12, fontFamily: 'inherit', whiteSpace: 'nowrap', opacity: squareConnecting ? .5 : 1, transition: 'opacity .2s' }}>
      ...
      {toast && (
        <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', background: 'rgba(8,8,8,.92)', border: '1px solid rgba(255,255,255,.14)', borderRadius: 999, padding: '10px 20px', boxShadow: '0 20px 60px rgba(0,0,0,.55)', display: 'flex', alignItems: 'center', gap: 10, backdropFilter: 'blur(18px)', fontSize: 13, zIndex: 5000, whiteSpace: 'nowrap', color: '#e9e9e9', fontFamily: 'inherit' }}>
          <span style={{ width: 8, height: 8, borderRadius: 999, background: toast.includes('Error') || toast.includes('failed') ? '#ff6b6b' : '#8ff0b1', flexShrink: 0 }} />
          {toast}
        </div>
      )}
    </Shell>
