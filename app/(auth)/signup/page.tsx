...
export default function SignUp() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!email || !password || !businessName) { setError('All fields required'); return }
  ...
  return (
    <div style={card}>
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        ...
        <div style={{ fontSize: 13, color: 'rgba(255,255,255,.40)' }}>Create your workspace</div>
      </div>

      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div>
          <label style={lbl}>Business name</label>
          <input value={businessName} onChange={e => setBusinessName(e.target.value)} placeholder="My Barbershop" style={inp} />
        </div>
        <div>
      ...
      <div style={{ textAlign: 'center', marginTop: 20 }}>
        <a href="/signin" style={{ fontSize: 13, color: '#8b9aff', textDecoration: 'none' }}>
          Already have an account? Sign in
        </a>
      </div>

      <div style={{ textAlign: 'center', marginTop: 16, fontSize: 11, color: 'rgba(255,255,255,.25)', lineHeight: 1.5 }}>
        By signing up, you agree to VuriumBook&apos;s<br />
        <a href="#" style={{ color: 'rgba(255,255,255,.35)', textDecoration: 'none' }}>Terms of Service</a> and{' '}
        <a href="#" style={{ color: 'rgba(255,255,255,.35)', textDecoration: 'none' }}>Privacy Policy</a>
      </div>
    </div>
  )
}
