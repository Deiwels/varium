...
Цей doc написаний **AI 4 (Phone AI)** на явний запит Owner'а. Зазвичай reviews і runbooks — територія **AI 3 (Verdent)** за [[AI-Work-Split]] і [[AI-Profiles/AI-3-Verdent|AI 3 — Verdent]]. Owner override acknowledged. Після цього ревю я (AI 4) повертаюся в standby; подальші updates цього документа робить AI 3 або AI 1.
...
| 6 | **Apple IAP Gap 5** — `autoProvisionSmsOnActivation` | Wired на 3 activation paths. Якщо Apple webhook падає з undefined `wsId` — новий paying customer не отримує TFN | Швидкий null-guard patch; revert якщо логіка глибша | AI 1 повний root-cause |
...
1. **Emergency runbook для `job_locks/` deadlock** — BE.1 це нова колекція; якщо треба manual-unlock, немає step-by-step.
   → **Пропоную AI 3 (Verdent) створити** `docs/Tasks/job_locks-Emergency-Runbook.md`
...
3. **On-call rotation / handoff protocol між AI 1 ↔ AI 4** — якщо AI 1 у процесі роботи і прод впав у той момент, хто приймає: я перехоплюю чи AI 1 перемикається? Manifesto описує рівні, але не координацію в real-time
   → **Пропоную AI 3 додати розділ "AI 4 activation protocol"** у [[AI-Core-Manifesto]] або у [[AI-Profiles/AI-4-Phone-AI|AI 4 — Phone AI]]
...
```text
1. git log --oneline -5
2. Читаю [[Web-Native-Auth-Contract]]
...
2. Firestore console → job_locks/
3. Якщо stale locks є — видаляю вручну (з дозволу Owner)
...
4. Payment-sensitive — явний "go" Owner'а required before push
...
**Launch readiness з моєї позиції:** 🟢 **green**

**Що перестерігає:**
- ⚠️ BE.1 — untested at multi-instance scale. Перший реальний Cloud Run scale-up буде stress-test
- ⚠️ Gaps #1 і #2 з секції 5 — варто закрити **до** launch marketing push, не після
- ⚠️ Owner SPOF для external items — якщо Owner зникне на кілька днів, частина задач застигне

**Жодних emergency hotfix'ів я зараз не виконую.** Інцидентів нема. Повертаюсь у standby.
...
|---|---|---|---|---|
| AI4-REQ.1 | Створити `docs/Tasks/job_locks-Emergency-Runbook.md` з manual unlock procedure для `job_locks/` колекції + явний `git revert 5dab7a1` fallback | AI 3 (Verdent) | ~10 min | High |
| AI4-REQ.2 | У наступних payment/auth/SMS-критичних commits додавати `Last-known-good SHA: <SHA>` рядок у commit message | AI 1 (Claude) | 0 min per commit | Medium |
| AI4-REQ.3 | Додати "AI 4 activation protocol" секцію у [[AI-Core-Manifesto]] або [[AI-Profiles/AI-4-Phone-AI|AI 4 — Phone AI]] | AI 3 (Verdent) | ~15 min | Medium |
| AI4-REQ.4 | Додати короткий список `requireSuperadmin` endpoints у [[API Routes]] або окремий `docs/Architecture/Superadmin-Endpoints.md` | AI 1 (Claude) | ~15 min | Low |
| AI4-REQ.5 | Додати секцію "Rotation protocol per secret" у [[Architecture/GitHub Secrets Inventory]] | AI 3 (Verdent) | ~15 min | Medium |
