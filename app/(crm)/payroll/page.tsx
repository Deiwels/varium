    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, backdropFilter: 'blur(8px)' }}
      ...
      <div style={{ ...s, width: 'min(560px,96vw)', borderRadius: 20, border: '1px solid rgba(255,255,255,.12)', background: 'linear-gradient(180deg,rgba(20,20,30,.90),rgba(10,10,20,.88))', backdropFilter: 'blur(24px)', padding: 18, boxShadow: '0 20px 60px rgba(0,0,0,.6)' }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
          {presets.map(p => (
            <button key={p.label} onClick={() => { onChange(p.f, p.t); onClose() }}
              style={{ ...btnBase, padding: '0 12px', fontSize: 11, letterSpacing: '.06em', background: selFrom === p.f && selTo === p.t ? 'rgba(139,154,255,.18)' : 'rgba(255,255,255,.04)', borderColor: selFrom === p.f && selTo === p.t ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.10)' }}>
              {p.label}
            </button>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 12, color: step === 'from' ? '#8b9aff' : '#8ff0b1', fontWeight: 700, letterSpacing: '.06em' }}>
          ...
          {days.map(iso => {
            ...
            return (
              <button key={iso} onClick={() => pickDay(iso)}
                style={{ height: 40, borderRadius: 10, border: `1px solid ${isFrom ? 'rgba(139,154,255,.75)' : isTo ? 'rgba(143,240,177,.65)' : isToday ? 'rgba(255,207,63,.50)' : 'rgba(255,255,255,.08)'}`, background: isFrom ? 'rgba(139,154,255,.20)' : isTo ? 'rgba(143,240,177,.15)' : inRange ? 'rgba(139,154,255,.08)' : 'rgba(0,0,0,.18)', color: isTo ? '#c9ffe1' : '#fff', cursor: 'pointer', opacity: inMonth ? 1 : 0.3, fontWeight: isFrom || isTo ? 900 : 500, fontSize: 13, fontFamily: 'inherit' }}>
        ...
        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid rgba(255,255,255,.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: '#c8d0ff' }}>{selFrom && selTo ? fmtDate(selFrom) + ' → ' + fmtDate(selTo) : '—'}</div>
        </div>
      </div>
    </div>
  )
...
function CommissionEditor({ barber, rule, onSaved }: { barber: BarberPayroll; rule: Rule; onSaved: (r: Rule) => void }) {
  ...
  return (
    <div style={{ borderRadius: 14, border: `1px solid ${open ? 'rgba(139,154,255,.30)' : 'rgba(255,255,255,.08)'}`, overflow: 'hidden', background: open ? 'rgba(139,154,255,.04)' : 'rgba(0,0,0,.12)' }}>
      ...
      {open && (
        <div style={{ padding: '12px 14px', borderTop: '1px solid rgba(255,255,255,.08)' }}>
          ...
          <button onClick={save} disabled={saving}
            style={{ width: '100%', height: 40, borderRadius: 12, border: '1px solid rgba(139,154,255,.55)', background: saved ? 'rgba(143,240,177,.12)' : 'rgba(139,154,255,.14)', color: saved ? '#c9ffe1' : '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', borderColor: saved ? 'rgba(143,240,177,.45)' : 'rgba(139,154,255,.55)' }}>
            ...
            <div className="day-pills" style={{ display: 'flex', gap: 6 }}>
              {DAY_LABELS.map((d, i) => (
                <button key={i} onClick={() => toggleBonusDay(i)}
                  style={{ height: 30, width: 40, borderRadius: 8, border: `1px solid ${bonusDays.includes(i) ? 'rgba(139,154,255,.45)' : 'rgba(255,255,255,.10)'}`, background: bonusDays.includes(i) ? 'rgba(139,154,255,.12)' : 'rgba(255,255,255,.03)', color: bonusDays.includes(i) ? '#c8d0ff' : 'rgba(255,255,255,.40)', cursor: 'pointer', fontSize: 10, fontWeight: 700, fontFamily: 'inherit' }}>
                  {d}
                </button>
              ))}
            </div>
...
export default function PayrollPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      try { localResets = JSON.parse(localStorage.getItem('VURIUMBOOK_LATE_RESETS') || '{}') } catch {}
      attRecords.forEach((r: any) => {
          ...
          <div className="payroll-topbar-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            ...
            <div className="payroll-topbar-actions" style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <button onClick={exportCSV}
                style={{ height: 40, padding: '0 16px', borderRadius: 999, border: '1px solid rgba(139,154,255,.65)', background: 'rgba(139,154,255,.12)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit' }}>
                Export CSV
              </button>
            </div>
          </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {visible.map(b => {
                      ...
                      return <>
                        <tr key={b.barber_id} style={{ background: isOpen ? 'rgba(139,154,255,.04)' : 'transparent' }}
                          onMouseEnter={e => (e.currentTarget.style.background='rgba(255,255,255,.025)')}
                          onMouseLeave={e => (e.currentTarget.style.background=isOpen?'rgba(139,154,255,.04)':'transparent')}>
                          ...
                          <td style={{ padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,.06)', whiteSpace: 'nowrap' }}>
                            {(() => { const mins = adminAttendance[b.barber_id] || 0; const h = mins / 60; return h > 0 ? <span style={{ fontSize: 12, color: '#c8d0ff', fontWeight: 700 }}>{h.toFixed(1)}h</span> : <span style={{ fontSize: 11, color: 'rgba(255,255,255,.25)' }}>—</span> })()}
                          </td>
                          <td style={{ padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,.06)', whiteSpace: 'nowrap' }}>
                            <span style={{ fontSize: 11, padding: '3px 9px', borderRadius: 999, border: isBoosted ? '1px solid rgba(143,240,177,.45)' : '1px solid rgba(139,154,255,.40)', background: isBoosted ? 'rgba(143,240,177,.10)' : 'rgba(139,154,255,.10)', color: isBoosted ? '#c9ffe1' : '#c8d0ff' }}>
                          ...
                          <td style={{ padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,.06)', color: '#c8d0ff' }}>{fmtMoney(b.barber_service_share)}</td>
                                ...
                                <div style={{ fontSize: 10, color: '#ff6b6b', fontWeight: 600, marginTop: 2, display: 'flex', alignItems: 'center', gap: 6 }}>
                                  ...
                                  <button onClick={async e => {
                                    ...
                                    try {
                                      ...
                                      try { const key = 'VURIUMBOOK_LATE_RESETS'; const resets = JSON.parse(localStorage.getItem(key) || '{}'); resets[b.barber_id] = resetTime; localStorage.setItem(key, JSON.stringify(resets)) } catch {}
              ...
              <div style={{ display: 'flex', gap: 6, padding: '12px 14px 0', flexWrap: 'wrap' }}>
                {(['summary','attendance','rules'] as const).map(tab => (
                  <button key={tab} onClick={() => setActiveTab(tab)}
                    style={{ height: 34, padding: '0 14px', borderRadius: 999, border: `1px solid ${activeTab===tab ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.10)'}`, background: activeTab===tab ? 'rgba(139,154,255,.14)' : 'rgba(255,255,255,.04)', color: activeTab===tab ? '#c8d0ff' : 'rgba(255,255,255,.70)', cursor: 'pointer', fontWeight: 900, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.08em', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                            ...
                            {adminCalcs.map(({ u, r, hours, basePay, profitShare, feeShare, total, allFeeDays }) => (
                              <div key={u.id} style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
                                ...
                                <div className="admin-payroll-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, fontSize: 12 }}>
                                  <div><span style={{ color: 'rgba(255,255,255,.40)' }}>Hours: </span><span style={{ color: '#c8d0ff' }}>{hours.toFixed(1)}h</span></div>
              ...
              {activeTab === 'attendance' && (
                <div style={{ padding: '12px 14px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#c8d0ff" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
