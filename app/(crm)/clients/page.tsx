...
const STATUS_STYLE: Record<string, React.CSSProperties> = {
  ...
  new:     { borderColor:'rgba(139,154,255,.45)', background:'rgba(139,154,255,.10)', color:'#c8d0ff' },
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.55)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100, backdropFilter:'blur(10px)' }}
      ...
      <div style={{ width:'min(420px,95vw)', borderRadius:20, border:'1px solid rgba(255,255,255,.12)', background:'linear-gradient(180deg,rgba(20,20,30,.92),rgba(10,10,20,.90))', backdropFilter:'blur(24px)', padding:20, color:'#e9e9e9', fontFamily:'Inter,sans-serif', boxShadow:'0 24px 80px rgba(0,0,0,.7)' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <button onClick={save} disabled={saving} style={{ height:44, borderRadius:12, border:'1px solid rgba(139,154,255,.65)', background:'rgba(139,154,255,.14)', color:'#c8d0ff', cursor:'pointer', fontWeight:900, fontSize:13, fontFamily:'inherit' }}>
    ...
    <div style={{ padding:14, display:'flex', flexDirection:'column', gap:10 }}>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
        {c.phone && row('Phone',
          isOwner
            ? (revealedPhones[c.id]
              ? <a href={`tel:${revealedPhones[c.id]}`} style={{ color:'#c8d0ff', textDecoration:'none' }}>{revealedPhones[c.id]}</a>
              : <span onClick={() => revealPhone(c.id, c.phone!)} style={{ color:'rgba(139,154,255,.8)', cursor:'pointer', fontSize:12 }}>{maskPhone(c.phone)} · tap to reveal</span>)
            : isAdmin
              ? (revealedPhones[c.id]
                ...
                : phoneLoading === c.id
                  ...
                  : <button onClick={() => requestPhone(c.id)} style={{ height:28, padding:'0 10px', borderRadius:999, border:'1px solid rgba(139,154,255,.45)', background:'rgba(139,154,255,.12)', color:'#c8d0ff', cursor:'pointer', fontWeight:700, fontSize:10, fontFamily:'inherit' }}>
        ...
        {c.email && row('Email', <a href={`mailto:${c.email}`} style={{ color:'#c8d0ff', textDecoration:'none', overflow:'hidden', textOverflow:'ellipsis', maxWidth:160, display:'block' }}>{c.email}</a>)}
        ...
        <div style={{ display:'flex', gap:6, flexWrap:'wrap' as const }}>
          {(['vip','active','new','risk'] as const).map(s => (
            <button key={s} onClick={() => patch({ status:s })}
              style={{ height:32, padding:'0 14px', borderRadius:999, border:`1px solid ${c.status===s ? (STATUS_STYLE[s]?.borderColor||'rgba(139,154,255,.55)') : 'rgba(255,255,255,.12)'}`, background:c.status===s ? (STATUS_STYLE[s]?.background||'rgba(139,154,255,.12)') : 'rgba(255,255,255,.04)', color:c.status===s ? (STATUS_STYLE[s]?.color||'#fff') : 'rgba(255,255,255,.65)', cursor:'pointer', fontWeight:700, fontSize:11, fontFamily:'inherit' }}>
              {STATUS_LABELS[s]}
            </button>
          ))}
        </div>
      ...
      <div>
        ...
        <div style={{ display:'flex', gap:8, marginTop:8 }}>
          <button onClick={saveNotes} disabled={notesSaving}
            style={{ height:34, padding:'0 16px', borderRadius:999, border:'1px solid rgba(139,154,255,.55)', background:'rgba(139,154,255,.12)', color:'#c8d0ff', cursor:'pointer', fontWeight:900, fontSize:12, fontFamily:'inherit' }}>
        ...
        return (
          <div>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:8 }}>
              ...
              <label style={{ height:28, padding:'0 10px', borderRadius:999, border:'1px solid rgba(139,154,255,.45)', background:'rgba(139,154,255,.10)', color:'#c8d0ff', cursor:'pointer', fontWeight:700, fontSize:10, fontFamily:'inherit', display:'flex', alignItems:'center', gap:4 }}>
  ...
  return (
    <Shell page="clients">
      <style>{`
        ...
        .cl-row.sel td{background:rgba(139,154,255,.07)!important}
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, flexWrap:'wrap' as const, marginBottom:10, position:'relative' }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' as const, alignItems:'center', position:'relative', zIndex:1 }}>
              <button onClick={() => setShowAdd(true)}
                style={{ height:40, padding:'0 16px', borderRadius:999, border:'1px solid rgba(139,154,255,.75)', background:'rgba(0,0,0,.75)', color:'#c8d0ff', cursor:'pointer', fontWeight:900, fontSize:13, fontFamily:'inherit', boxShadow:'0 0 18px rgba(139,154,255,.25)' }}>
                + Add client
              </button>
            </div>
          </div>
          ...
          <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center', marginTop:8 }}>
            {[{v:'',l:'All'},{v:'vip',l:'VIP'},{v:'active',l:'Active'},{v:'new',l:'New'},{v:'at_risk',l:'At Risk'}].map(f => (
              <button key={f.v} onClick={() => setFilterClientStatus(f.v)}
                style={{ height:30, padding:'0 12px', borderRadius:999, border:`1px solid ${filterClientStatus===f.v ? 'rgba(139,154,255,.65)' : 'rgba(255,255,255,.12)'}`, background:filterClientStatus===f.v ? 'rgba(139,154,255,.14)' : 'rgba(255,255,255,.04)', color:filterClientStatus===f.v ? '#c8d0ff' : 'rgba(255,255,255,.55)', cursor:'pointer', fontWeight:700, fontSize:11, fontFamily:'inherit', letterSpacing:'.04em' }}>
                {f.l}
              </button>
            ))}
          </div>
                              ...
                              <div style={{ fontSize:11, color:'rgba(255,255,255,.45)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', marginTop:1 }}>
                                {isOwner
                                  ? (revealedPhones[c.id]
                                    ...
                                    : <span onClick={() => revealPhone(c.id, c.phone||'')} style={{ cursor:'pointer', color:'rgba(139,154,255,.8)' }}>{maskPhone(c.phone||'')}</span>)
                                  : isAdmin
                                    ? (revealedPhones[c.id]
                                      ? <span style={{ color:'#8ff0b1' }}>{revealedPhones[c.id]}</span>
                                      : phoneLoading === c.id
                                        ? <span style={{ color:'rgba(255,255,255,.40)', fontSize:10 }}>Verifying...</span>
                                        : <span onClick={() => requestPhone(c.id)} style={{ cursor:'pointer', color:'rgba(139,154,255,.8)', fontSize:10 }}>{maskPhone(c.phone||'')} · request</span>)
                                    : maskPhone(c.phone||'')}
                              </div>
                            </div>
                          </div>
