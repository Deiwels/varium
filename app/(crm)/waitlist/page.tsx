      ...
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 16, height: '100vh', overflowY: 'auto' }}>
        ...
        <div className="wl-header-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', gap: 8, position: 'relative', zIndex: 1 }}>
            ...
            <button onClick={() => setAdding(!adding)} style={{ height: 36, padding: '0 14px', borderRadius: 999, border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.12)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 700, fontSize: 12, fontFamily: 'inherit' }}>
              {adding ? 'Cancel' : '+ Add'}
            </button>
          </div>
        </div>
        ...
        <div className="wl-filter-pills" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          ...
          {barbers.map(b => (
            <button key={b.id} onClick={() => setFilter(b.id)}
              style={{ height: 32, padding: '0 12px', borderRadius: 999, border: `1px solid ${filter === b.id ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.10)'}`, background: filter === b.id ? 'rgba(139,154,255,.12)' : 'transparent', color: filter === b.id ? '#c8d0ff' : 'rgba(255,255,255,.65)', cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: 'inherit' }}>
              {b.name}
            </button>
          ))}
        </div>
        ...
        {/* Add form */}
        {adding && (
          <div style={{ padding: 16, borderRadius: 18, border: '1px solid rgba(139,154,255,.25)', background: 'rgba(139,154,255,.06)', display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#c8d0ff', letterSpacing: '.08em', textTransform: 'uppercase' }}>Add to waitlist</div>
              ...
              <>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                    ...
                    />
                    {phoneSearching && <div style={{ position: 'absolute', right: 14, width: 16, height: 16, border: '2px solid rgba(255,255,255,.20)', borderTop: '2px solid #8b9aff', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />}
                  </div>
                </div>
                  ...
                  <div style={{ borderRadius: 14, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(255,255,255,.05)', overflow: 'hidden' }}>
                    {foundClients.map(c => (
                      <div key={c.id} onClick={() => selectClient(c)}
                        ...
                        onMouseEnter={e => (e.currentTarget.style.background = 'rgba(139,154,255,.10)')}
                          ...
                          return (
                            <button key={i} type="button" disabled={disabled}
                              ...
                              style={{ height: 36, borderRadius: 999, border: 'none', cursor: disabled ? 'default' : 'pointer', fontSize: 13, fontWeight: 700, fontFamily: 'inherit',
                                background: isSel ? 'rgba(139,154,255,.22)' : 'transparent',
                                color: disabled ? 'rgba(255,255,255,.18)' : isSel ? '#fff' : c.inMonth ? '#e9e9e9' : 'rgba(255,255,255,.18)',
                                boxShadow: isSel ? '0 0 0 2px rgba(139,154,255,.75) inset, 0 0 14px rgba(139,154,255,.30)' : isToday && c.inMonth ? '0 0 0 1px rgba(255,255,255,.20) inset' : 'none',
                              }}>
                              {c.day}
                            </button>
                          )
                ...
                <div className="wl-services-wrap" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {services.filter((s: any) => !s.barber_ids?.length || s.barber_ids.includes(newBarberId)).map((s: any) => (
                    <button key={s.id} onClick={() => setNewServiceIds(prev => prev.includes(s.id) ? prev.filter(x => x !== s.id) : [...prev, s.id])}
                      style={{ height: 32, padding: '0 12px', borderRadius: 999, border: `1px solid ${newServiceIds.includes(s.id) ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.12)'}`, background: newServiceIds.includes(s.id) ? 'rgba(139,154,255,.14)' : 'rgba(255,255,255,.04)', color: newServiceIds.includes(s.id) ? '#c8d0ff' : 'rgba(255,255,255,.60)', cursor: 'pointer', fontSize: 12, fontWeight: 600, fontFamily: 'inherit' }}>
            ...
            <button onClick={addEntry} disabled={saving || (!(selectedClient?.name || newName.trim())) || !newBarberId}
              style={{ height: 44, borderRadius: 12, border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.14)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', opacity: saving ? .5 : 1 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'rgba(255,255,255,.45)', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                      ...
                      {(entry as any).preferred_start_min != null && (entry as any).preferred_end_min != null && (entry as any).preferred_end_min < 1440 && (
                        <span style={{ color: 'rgba(139,154,255,.70)' }}>
