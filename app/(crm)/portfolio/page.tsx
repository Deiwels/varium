...
function PhotoEditor({ src, onSave, onClose }: { src: string; onSave: (dataUrl: string) => void; onClose: () => void }) {
  ...
  const pill = (active: boolean): React.CSSProperties => ({ height: 32, padding: '0 12px', borderRadius: 999, border: `1px solid ${active ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.12)'}`, background: active ? 'rgba(139,154,255,.14)' : 'rgba(255,255,255,.04)', color: active ? '#c8d0ff' : 'rgba(255,255,255,.65)', cursor: 'pointer', fontWeight: 700, fontSize: 10, letterSpacing: '.06em', textTransform: 'uppercase' as const, fontFamily: 'inherit', transition: 'all .2s', display: 'inline-flex', alignItems: 'center', gap: 6 })

  const sliderStyle: React.CSSProperties = { width: '100%', accentColor: '#8b9aff', height: 4, background: 'rgba(255,255,255,.08)', borderRadius: 999, cursor: 'pointer' }
      ...
      <div style={{ padding: '10px 16px 16px', flexShrink: 0, minHeight: 80 }}>
        {tool === 'filter' && (
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            {FILTERS.map(f => (
              <button key={f.id} onClick={() => setFilter(f.id)} style={{ flexShrink: 0, width: 58, height: 58, borderRadius: 12, overflow: 'hidden', border: `2px solid ${filter === f.id ? 'rgba(139,154,255,.65)' : 'rgba(255,255,255,.08)'}`, background: '#000', cursor: 'pointer', position: 'relative', padding: 0, boxShadow: filter === f.id ? '0 0 10px rgba(139,154,255,.25)' : 'none', transition: 'all .2s' }}>
                <img src={src} alt={f.label} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: f.css || 'none' }} />
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(0,0,0,.75)', padding: '2px 0', fontSize: 7, fontWeight: 700, letterSpacing: '.04em', textAlign: 'center', color: filter === f.id ? '#c8d0ff' : 'rgba(255,255,255,.55)' }}>{f.label}</div>
              </button>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#000', color: '#e9e9e9', fontFamily: 'Inter,system-ui,sans-serif' }}>
        ...
        <div style={{ padding: '14px 20px', background: 'rgba(0,0,0,.80)', backdropFilter: 'blur(14px)', borderBottom: '1px solid rgba(255,255,255,.08)', position: 'sticky', top: 0, zIndex: 20, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <label style={{
            ...
            border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.10)',
            color: '#c8d0ff', cursor: uploading ? 'wait' : 'pointer',
            fontWeight: 900, fontSize: 12, fontFamily: 'inherit',
            display: 'inline-flex', alignItems: 'center', gap: 8,
            boxShadow: '0 0 14px rgba(139,154,255,.20)',
          }}>
                  ...
                  <div className="port-overlay" style={{
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button onClick={e => { e.stopPropagation(); setEditingPhoto({ index: i, src: url }) }}
                        style={{ width: 28, height: 28, borderRadius: 8, border: '1px solid rgba(139,154,255,.35)', background: 'rgba(139,154,255,.15)', color: '#c8d0ff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
