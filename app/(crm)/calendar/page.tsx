...
const END_HOUR = 24
const COL_MIN = 190
const BARBER_COLORS = ['#99d100','#a86bff','#8b9aff','#ffb000','#ff5aa5','#35d6c7','#ff6b6b']
...
const STATUS_COLORS: Record<string, { border: string; bg: string; color: string }> = {
  ...
  booked:    { border: 'rgba(139,154,255,.40)',  bg: 'rgba(139,154,255,.10)',  color: '#c8d0ff' },
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 4, margin: '6px auto' }}>
      {DAY_NAMES.map((name, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 3, border: `1px solid ${day.enabled ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.10)'}`, borderRadius: 10, padding: '5px 4px', background: day.enabled ? 'rgba(139,154,255,.08)' : 'rgba(0,0,0,.18)', opacity: day.enabled ? 1 : 0.55 }}>
            ...
            <button onClick={() => toggle(i)} style={{ height: 22, borderRadius: 999, border: `1px solid ${day.enabled ? 'rgba(139,154,255,.65)' : 'rgba(255,255,255,.16)'}`, background: day.enabled ? 'rgba(139,154,255,.16)' : 'rgba(255,255,255,.05)', color: day.enabled ? '#c8d0ff' : '#fff', cursor: 'pointer', fontSize: 8, textTransform: 'uppercase', fontWeight: 900, fontFamily: 'inherit', width: '100%' }}>{day.enabled ? 'ON' : 'OFF'}</button>
        ...
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 12, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(255,255,255,.04)' }}>
          {editingIdx === i ? (
            <input
              ...
              style={{ width: 70, height: 24, borderRadius: 6, border: '1px solid rgba(139,154,255,.50)', background: 'rgba(139,154,255,.10)', color: '#fff', padding: '0 6px', outline: 'none', fontSize: 10, fontWeight: 900, letterSpacing: '.08em', fontFamily: 'inherit' }}
            />
          ) : (
            <span
          ...
          <input
            ...
            style={{ flex: 1, height: 4, accentColor: '#8b9aff', cursor: 'pointer' }}
          />
...
function BarberEditCard({ b, onDelete, onSaved, onError, isBarberSelf }: {
  ...
  return (
    <div className="barber-edit-card" style={{ borderRadius: 16, border: `1px solid ${open ? 'rgba(139,154,255,.35)' : 'rgba(255,255,255,.10)'}`, background: open ? 'rgba(139,154,255,.08)' : 'rgba(255,255,255,.03)', transition: 'border-color .25s ease, box-shadow .25s ease' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 14px' }}>
            ...
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,.40)', letterSpacing: '.06em', marginTop: 2, display: 'flex', alignItems: 'center', gap: 6 }}>
              {(() => {
                ...
                const badgeColors = lvl.includes('ambassador')
                  ...
                  : lvl.includes('senior')
                  ? { border: 'rgba(139,154,255,.45)', bg: 'rgba(139,154,255,.10)', color: '#c8d0ff', shadow: '0 0 8px rgba(139,154,255,.20)' }
                  : lvl.includes('expert')
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          <button onClick={() => setOpen(v => !v)} style={{ height: 36, padding: '0 14px', borderRadius: 999, border: `1px solid ${open ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.14)'}`, background: open ? 'rgba(139,154,255,.12)' : 'rgba(255,255,255,.05)', color: open ? '#c8d0ff' : '#fff', cursor: 'pointer', fontWeight: 900, fontSize: 12, fontFamily: 'inherit' }}>{open ? 'Collapse' : 'Edit'}</button>
      ...
      <div style={{ maxHeight: open ? 2000 : 0, opacity: open ? 1 : 0, overflow: 'hidden', transition: 'max-height .4s ease, opacity .3s ease' }}>
        <div style={{ padding: '0 14px 14px', borderTop: open ? '1px solid rgba(255,255,255,.08)' : '1px solid transparent', overflow: 'hidden' }}>
          ...
          <button onClick={save} disabled={saving} style={{ width: '100%', height: 42, borderRadius: 12, border: '1px solid rgba(139,154,255,.65)', background: 'rgba(139,154,255,.10)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', marginTop: 12 }}>
          ...
          {tab === 'barbers' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {!isBarber && <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: 16 }}>
                ...
                <button onClick={addBarber} disabled={saving} style={{ width: '100%', height: 42, borderRadius: 12, border: '1px solid rgba(139,154,255,.65)', background: 'rgba(139,154,255,.10)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', marginTop: 12 }}>
          ...
          {tab === 'services' && (
            <div>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px' }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            <span style={{ fontSize: 8, fontWeight: 800, letterSpacing: '.06em', textTransform: 'uppercase', padding: '2px 6px', borderRadius: 4, border: `1px solid ${s.service_type === 'addon' ? 'rgba(255,207,63,.35)' : 'rgba(139,154,255,.35)'}`, background: s.service_type === 'addon' ? 'rgba(255,207,63,.12)' : 'rgba(139,154,255,.12)', color: s.service_type === 'addon' ? '#ffe9a3' : '#c8d0ff' }}>{s.service_type === 'addon' ? 'Add-on' : 'Primary'}</span>
                          </div>
                            ...
                            <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                              {(['primary', 'addon'] as const).map(t => (
                                <button key={t} onClick={() => setSType(t)}
                                  style={{ height: 30, padding: '0 12px', borderRadius: 999, border: `1px solid ${sType === t ? (t === 'addon' ? 'rgba(255,207,63,.40)' : 'rgba(139,154,255,.40)') : 'rgba(255,255,255,.10)'}`, background: sType === t ? (t === 'addon' ? 'rgba(255,207,63,.12)' : 'rgba(139,154,255,.12)') : 'rgba(255,255,255,.03)', color: sType === t ? (t === 'addon' ? '#ffe9a3' : '#c8d0ff') : 'rgba(255,255,255,.45)', cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: 'inherit', transition: 'all .2s' }}>
              ...
              {!isBarber && <div style={{ borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: 14 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                      {(['primary', 'addon'] as const).map(t => (
                        <button key={t} onClick={() => { if (!editSvcId) setSType(t) }}
                          style={{ height: 30, padding: '0 12px', borderRadius: 999, border: `1px solid ${!editSvcId && sType === t ? (t === 'addon' ? 'rgba(255,207,63,.40)' : 'rgba(139,154,255,.40)') : 'rgba(255,255,255,.10)'}`, background: !editSvcId && sType === t ? (t === 'addon' ? 'rgba(255,207,63,.12)' : 'rgba(139,154,255,.12)') : 'rgba(255,255,255,.03)', color: !editSvcId && sType === t ? (t === 'addon' ? '#ffe9a3' : '#c8d0ff') : 'rgba(255,255,255,.45)', cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: 'inherit', opacity: editSvcId ? 0.4 : 1, transition: 'all .2s' }}>
                          {t === 'primary' ? 'Primary' : 'Add-on'}
                        </button>
                      ))}
                    </div>
                ...
                <button onClick={addService} disabled={saving || !!editSvcId} style={{ width: '100%', height: 42, borderRadius: 12, border: '1px solid rgba(139,154,255,.65)', background: 'rgba(139,154,255,.10)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', opacity: editSvcId ? 0.4 : 1 }}>
...
export default function CalendarPage() {
  ...
  const ARRIVED_KEY = 'VURIUMBOOK_ARRIVED_IDS'
  ...
  const [studentSchedule, setStudentSchedule] = useState<DaySchedule[]>(() => {
    try { const s = localStorage.getItem('VURIUMBOOK_STUDENT_SCHEDULE'); if (s) return JSON.parse(s) } catch {}
    return DAY_DEFAULTS.map(d => ({...d}))
  })
    ...
    ;(async () => {
      try {
        ...
        if (Array.isArray(sched) && sched.length === 7) {
          ...
          localStorage.setItem('VURIUMBOOK_STUDENT_SCHEDULE', JSON.stringify(sched))
        }
      } catch {}
    })()
  }, [isStudent])
    ...
    <Shell page="calendar">
      ...
      <style>{`
        ...
        @keyframes wlGhostPulse {
          0%, 100% { box-shadow: 0 0 8px rgba(139,154,255,.15), inset 0 0 0 1px rgba(139,154,255,.12); border-color: rgba(139,154,255,.25); background: rgba(139,154,255,.05); }
          50% { box-shadow: 0 0 22px rgba(139,154,255,.45), inset 0 0 0 1px rgba(139,154,255,.30); border-color: rgba(139,154,255,.55); background: rgba(139,154,255,.12); }
        }
        ...
        .arrived-pulse {
          border: 1px solid rgba(139,154,255,.30);
          background: linear-gradient(90deg, rgba(139,154,255,.04) 0%, rgba(139,154,255,.18) 25%, rgba(139,154,255,.30) 50%, rgba(139,154,255,.18) 75%, rgba(139,154,255,.04) 100%);
          background-size: 200% 100%;
          animation: arrivedShimmer 2.4s ease-in-out infinite;
          box-shadow: 0 0 10px rgba(139,154,255,.12), inset 0 0 8px rgba(139,154,255,.06);
        }
        ...
        @keyframes dragLift {
          ...
          100% { transform: scale(1.06) perspective(600px) rotateX(0deg); box-shadow: 0 16px 40px rgba(0,0,0,.50), 0 0 20px rgba(139,154,255,.30); }
        }
        .cal-event-dragging {
          animation: dragLift .2s ease-out forwards !important;
          border: 1.5px solid rgba(139,154,255,.70) !important;
          z-index: 50 !important;
          opacity: 1 !important;
          filter: brightness(1.15);
        }
        .cal-event-drag-ghost {
          border: 2px dashed rgba(139,154,255,.50) !important;
          background: rgba(139,154,255,.08) !important;
          box-shadow: 0 0 16px rgba(139,154,255,.20), inset 0 0 12px rgba(139,154,255,.06);
        }
        ...
        input[type=range]::-webkit-slider-thumb { -webkit-appearance: none; width: 14px; height: 14px; border-radius: 50%; background: #8b9aff; cursor: pointer; border: 2px solid rgba(0,0,0,.40); box-shadow: 0 1px 4px rgba(0,0,0,.3); }
        input[type=range]::-moz-range-thumb { width: 14px; height: 14px; border-radius: 50%; background: #8b9aff; cursor: pointer; border: 2px solid rgba(0,0,0,.40); box-shadow: 0 1px 4px rgba(0,0,0,.3); }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: '#000', color: '#e9e9e9', fontFamily: 'Inter,system-ui,sans-serif' }}>
          ...
          <div className="cal-topbar-row" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
            ...
            <div className="cal-topbar-btns" style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {isStudent ? (
                ...
                <button className="cal-new-btn" onClick={() => openCreate(isBarber ? myBarberId : (barbers[0]?.id || ''), clamp(new Date().getHours()*60))} style={{ height: 36, padding: '0 12px', borderRadius: 999, border: '1px solid rgba(139,154,255,.80)', background: 'rgba(0,0,0,.75)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 12, fontFamily: 'inherit', boxShadow: '0 0 14px rgba(139,154,255,.20)', whiteSpace: 'nowrap', flexShrink: 0 }}>+ New</button>
              )}
              ...
              {pageBarbers.map((barber, bi) => {
                ...
                return (
                  <div key={barber.id} ref={el => { colRefs.current[bi] = el }}
                    style={{ position: 'relative', borderRight: bi < visibleBarbers.length-1 ? '1px solid rgba(255,255,255,.08)' : 'none', background: blockDrag?.barberIdx === bi ? 'rgba(255,107,107,.03)' : drag?.ghostBarberIdx === bi ? 'rgba(139,154,255,.03)' : 'transparent', transition: 'background .15s', touchAction: (drag || blockDrag) ? 'none' : 'pan-y' }}
                    onMouseDown={e => {
                    ...
                    {showNow && (
                      <div style={{ position: 'absolute', left: 0, right: 0, top: nowY, height: 2, background: 'rgba(139,154,255,.85)', boxShadow: '0 0 14px rgba(139,154,255,.40)', pointerEvents: 'none', zIndex: 20 }}>
                        {bi === 0 && <div style={{ position: 'absolute', left: -4, top: -4, width: 10, height: 10, borderRadius: 999, background: '#8b9aff', boxShadow: '0 0 0 3px rgba(139,154,255,.25)' }} />}
                      </div>
                    )}
                    ...
                    {touchIndicator && bi === 0 && (() => {
                      ...
                      return (
                        <>
                          {/* Highlight row */}
                          <div style={{ position: 'absolute', left: -timeColW, right: -(pageBarbers.length - 1) * colMin, top: indY - slotH * 3, height: slotH * 6, background: 'rgba(139,154,255,.04)', pointerEvents: 'none', zIndex: 1 }} />
                          {/* Crosshair line */}
                          <div style={{ position: 'absolute', left: -timeColW, right: -(pageBarbers.length - 1) * colMin, top: indY, height: 1, background: 'rgba(139,154,255,.35)', pointerEvents: 'none', zIndex: 25, boxShadow: '0 0 6px rgba(139,154,255,.25)' }} />
                          {/* Time tooltip */}
                          <div style={{ position: 'absolute', left: -timeColW, top: indY - 11, zIndex: 26, pointerEvents: 'none', background: 'rgba(139,154,255,.85)', borderRadius: 6, padding: '2px 6px', fontSize: 9, fontWeight: 800, color: '#fff', whiteSpace: 'nowrap' }}>
                          ...
                          {height > 50 && ((ev._raw?.reference_photo_url || ev._raw?.client_photo || ev._raw?.haircut_photo || ev._raw?.photo_url || ev._raw?.attachment_url) || (ev.notes && ev.notes.replace(/Reference photo attached on website:\s*\S+/gi, '').trim())) && (
                            <div style={{ marginTop: 3, display: 'flex', alignItems: 'center', gap: 3 }}>
                              {(ev._raw?.reference_photo_url || ev._raw?.client_photo || ev._raw?.haircut_photo || ev._raw?.photo_url || ev._raw?.attachment_url) && (
                                <span title="Has reference photo" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 16, height: 16, borderRadius: 5, background: 'rgba(139,154,255,.18)', border: '1px solid rgba(139,154,255,.30)', flexShrink: 0 }}>
                    ...
                    {!isStudent && waitlistEntries.filter(w => w.barber_id === barber.id).map((w, wi) => {
                      ...
                      return (
                        <div key={`wl-${w.id}`} className="wl-ghost-pulse" style={{ position: 'absolute', left: 8, right: 8, top, height, borderRadius: 14, border: '1px solid rgba(139,154,255,.35)', background: 'rgba(139,154,255,.08)', zIndex: 3, padding: '6px 10px', cursor: 'pointer', overflow: 'hidden', boxShadow: '0 0 12px rgba(139,154,255,.20), inset 0 0 0 1px rgba(139,154,255,.15)' }}
                          ...
                          <div style={{ fontSize: 9, color: 'rgba(139,154,255,.70)', marginTop: 1 }}>{minToAMPM(slotMin)} · {dur}min · {prefStart !== wh.startMin || prefEnd !== wh.endMin ? `${minToAMPM(prefStart)}-${minToAMPM(prefEnd)}` : 'WAITLIST'}</div>
                        </div>
                      )
                    })}
                  </div>
        ...
        {isMobile && (() => {
          ...
          return (
            <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 15, padding: '6px 20px max(6px, env(safe-area-inset-bottom, 6px))', display: 'flex', justifyContent: 'space-between', alignItems: 'center', pointerEvents: 'auto' }}>
              {/* Settings button */}
              <button onClick={() => setSettingsOpen(true)} style={{ width: 28, height: 28, borderRadius: 999, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.50)', color: 'rgba(255,255,255,.40)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              {dots.slice(1, -1).map((dot, i) => (
                <button key={dot.day + '-' + dot.date.getMonth()}
                  ...
                  {dot.isToday && !dot.isCurrent && <div style={{ position: 'absolute', bottom: 3, width: 3, height: 3, borderRadius: 999, background: 'rgba(139,154,255,.90)' }} />}
                </button>
              ))}
              </div>
              {/* + button */}
              ...
              }} style={{ width: 28, height: 28, borderRadius: 999, border: '1px solid rgba(139,154,255,.45)', background: 'rgba(0,0,0,.50)', color: '#c8d0ff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        {loading && dayTransition === 'idle' && !isMobile && <div style={{ position: 'fixed', bottom: 20, right: 20, padding: '8px 16px', borderRadius: 999, background: 'rgba(139,154,255,.20)', border: '1px solid rgba(139,154,255,.40)', color: '#c8d0ff', fontSize: 12, zIndex: 99 }}>Loading…</div>}
      </div>
      ...
      {contextMenu && (() => {
        ...
        const cmItems = [
          { label: 'Booking', icon: <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#c8d0ff" strokeWidth="2.2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>, bg: 'rgba(139,154,255,.18)', brd: 'rgba(139,154,255,.35)', col: '#c8d0ff', fn: () => { setContextMenu(null); openCreate(contextMenu.barberId, contextMenu.min) } },
          ...(hasStudents ? [{
          ...
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', backdropFilter: 'blur(18px)', WebkitBackdropFilter: 'blur(18px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
            <div style={{ width: 'min(380px,92vw)', borderRadius: 22, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.65)', backdropFilter: 'saturate(180%) blur(40px)', WebkitBackdropFilter: 'saturate(180%) blur(40px)', boxShadow: '0 32px 80px rgba(0,0,0,.55), inset 0 0 0 0.5px rgba(255,255,255,.06)', padding: 20, color: '#e9e9e9', fontFamily: 'Inter,sans-serif' }}>
              ...
              <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button onClick={confirmDragMove} style={{ height: 40, padding: '0 20px', borderRadius: 999, border: '1px solid rgba(139,154,255,.75)', background: 'rgba(139,154,255,.18)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontFamily: 'inherit', fontSize: 13 }}>Move</button>
              </div>
            </div>
          </div>
        )
      ...
      {settingsOpen && <SettingsModal barbers={barbers} services={services} onClose={() => setSettingsOpen(false)} onReload={reloadAll}
        ...
        studentSchedule={studentSchedule} onStudentScheduleChange={(s: DaySchedule[]) => {
          setStudentSchedule(s); localStorage.setItem('VURIUMBOOK_STUDENT_SCHEDULE', JSON.stringify(s))
      ...
      {wlConfirm && (() => {
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', backdropFilter: 'blur(18px)', WebkitBackdropFilter: 'blur(18px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 16 }}
            ...
            <div style={{ width: 'min(420px,92vw)', borderRadius: 22, border: '1px solid rgba(139,154,255,.25)', background: 'rgba(0,0,0,.70)', backdropFilter: 'saturate(180%) blur(40px)', WebkitBackdropFilter: 'saturate(180%) blur(40px)', boxShadow: '0 32px 80px rgba(0,0,0,.60), inset 0 0 0 0.5px rgba(255,255,255,.07)', padding: '24px 22px', color: '#e9e9e9', fontFamily: 'Inter,sans-serif' }}>
              {/* Icon */}
              <div style={{ textAlign: 'center', marginBottom: 16 }}>
                <div style={{ width: 52, height: 52, borderRadius: 16, background: 'rgba(139,154,255,.10)', border: '1px solid rgba(139,154,255,.30)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button onClick={doConfirm} disabled={wlConfirming}
                  style={{ flex: 2, height: 44, borderRadius: 999, border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.12)', color: '#bfe0ff', cursor: 'pointer', fontWeight: 900, fontFamily: 'inherit', fontSize: 13, opacity: wlConfirming ? .5 : 1 }}>
