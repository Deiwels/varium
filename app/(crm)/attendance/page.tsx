...
const ROLE_COLORS: Record<string, string> = { owner: '#ffe9a3', admin: '#c9ffe1', barber: '#c8d0ff', student: '#d4b8ff' }
...
export default function AttendancePage() {
  ...
  async function forceClockOut(attendanceId: string, userName: string, userId: string) {
    ...
    try {
      await apiFetch(`/api/attendance/admin-clock-out`, {
        method: 'POST',
                  ...
                  <div key={uid} style={{ padding: '8px 10px', borderRadius: 12, background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.06)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 16, fontSize: 12 }}>
                      ...
                      <div>
                        ...
                        <span style={{ color: '#c8d0ff', fontWeight: 700 }}>{u.shifts}</span>
                      </div>
                      {u.late_count > 0 && (
                        <div>
                    ...
                    <div style={{ marginTop: 6, height: 4, borderRadius: 999, background: 'rgba(255,255,255,.06)', overflow: 'hidden' }}>
                      <div style={{ height: '100%', borderRadius: 999, background: 'linear-gradient(90deg,rgba(139,154,255,.60),rgba(143,240,177,.60))', width: `${Math.min(100, (u.total_minutes / Math.max(1, summaryList[0]?.[1]?.total_minutes || 1)) * 100)}%`, transition: 'width .3s ease' }} />
                    </div>
                  </div>
                ))}
