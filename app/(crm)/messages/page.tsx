...
const TAB_COLORS: Record<string, string> = {
  general: '#c8d0ff', barbers: '#c8d0ff', admins: '#c9ffe1', students: '#d4b8ff', requests: '#ffe9a3', applications: '#ffb7d5'
}
...
const ROLE_COLORS: Record<string, string> = {
  owner: '#ffe9a3', admin: '#c9ffe1', barber: '#c8d0ff', student: '#d4b8ff'
}

const ROLE_GRADIENTS: Record<string, string> = {
  owner: 'linear-gradient(135deg, #ffe9a3, #f0c040)',
  admin: 'linear-gradient(135deg, #c9ffe1, #4ade80)',
  barber: 'linear-gradient(135deg, #c8d0ff, #60a5fa)',
...
function AudioPlayer({ src, isOwn }: { src: string; isOwn: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 180, padding: '2px 0' }}>
      <button onClick={toggle} style={{ width: 32, height: 32, borderRadius: 999, border: 'none', background: isOwn ? 'rgba(255,255,255,.20)' : 'rgba(139,154,255,.25)', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        {playing ? (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 1.5, height: 22 }}>
          {bars.map((h, i) => {
            ...
            return <div key={i} style={{ flex: 1, height: `${h * 100}%`, borderRadius: 1, background: filled ? (isOwn ? 'rgba(255,255,255,.80)' : 'rgba(139,154,255,.80)') : (isOwn ? 'rgba(255,255,255,.25)' : 'rgba(255,255,255,.15)'), transition: 'background .15s' }} />
          })}
        </div>
    ...
    <div className="msg-bubble-wrap" style={{ display: 'flex', flexDirection: isOwn ? 'row-reverse' : 'row', gap: isGrouped ? 0 : 10, alignItems: 'flex-end', marginBottom: isGrouped ? 2 : (hasReactions ? 28 : 8), padding: '0 16px', position: 'relative' }}>
      ...
      <div style={{ maxWidth: '72%', position: 'relative' }}
        ...
        <div style={{
          ...
          background: isOwn
            ? 'linear-gradient(135deg, rgba(139,154,255,.85), rgba(10,100,220,.90))'
            : 'rgba(255,255,255,.08)',
              ...
              return (
                <button key={emoji} onClick={() => onReaction?.(msg.id, emoji)}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: 3, height: 24, padding: '0 7px', borderRadius: 999,
                    border: `1px solid ${isMine ? 'rgba(139,154,255,.45)' : 'rgba(255,255,255,.12)'}`,
                    background: isMine ? 'rgba(139,154,255,.14)' : 'rgba(255,255,255,.06)',
                    cursor: 'pointer', fontSize: 12, color: '#e9e9e9', fontFamily: 'inherit',
                    transition: 'all .2s ease',
                  }}>
                  <span>{emoji}</span>
                  <span style={{ fontSize: 10, fontWeight: 700, color: isMine ? '#c8d0ff' : 'rgba(255,255,255,.50)' }}>{uids.length}</span>
                </button>
              )
            })}
          </div>
...
const REQ_TYPE_INFO: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  photo_change:    { label: 'Photo change', color: '#c8d0ff', icon: <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#c8d0ff" strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg> },
      ...
      <div style={{ width: 'min(440px,100%)', borderRadius: 22, border: '1px solid rgba(255,255,255,.10)', background: 'rgba(0,0,0,.65)', backdropFilter: 'saturate(180%) blur(40px)', boxShadow: '0 32px 80px rgba(0,0,0,.60)', color: '#e9e9e9', fontFamily: 'Inter,sans-serif' }}>
        ...
        <div style={{ padding: '16px 20px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {DAY_NAMES.map(d => (
                    <button key={d} onClick={() => setDays(prev => prev.includes(d) ? prev.filter(x=>x!==d) : [...prev, d])}
                      style={{ width: 42, height: 34, borderRadius: 8, border: `1px solid ${days.includes(d) ? 'rgba(139,154,255,.50)' : 'rgba(255,255,255,.10)'}`, background: days.includes(d) ? 'rgba(139,154,255,.14)' : 'rgba(255,255,255,.04)', color: days.includes(d) ? '#c8d0ff' : 'rgba(255,255,255,.40)', cursor: 'pointer', fontWeight: 700, fontSize: 11, fontFamily: 'inherit' }}>{d}</button>
                  ))}
                </div>
              </div>
              {/* Times */}
          ...
          <button onClick={submit} disabled={saving}
            style={{ height: 42, borderRadius: 12, border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.14)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', opacity: saving ? .5 : 1 }}>
  ...
  return (
    <Shell page="Messages">
      ...
      <style>{`
        ...
        .msg-input:focus { border-color: rgba(139,154,255,.40) !important; box-shadow: 0 0 0 3px rgba(139,154,255,.10) !important; }
        ...
        @keyframes sendGlow {
          0%, 100% { box-shadow: 0 0 0 0 rgba(139,154,255,0); }
          50% { box-shadow: 0 0 16px 4px rgba(139,154,255,.40); }
        }
              ...
              return (
                <button key={t.id} className="msg-tab" onClick={() => setActiveTab(t.id)}
                  style={{
                    ...
                    border: isActive ? '1px solid rgba(139,154,255,.40)' : '1px solid rgba(255,255,255,.08)',
                    background: isActive ? 'rgba(139,154,255,.20)' : 'rgba(255,255,255,.06)',
                    boxShadow: isActive ? '0 0 12px rgba(139,154,255,.15)' : 'none',
        ...
        {activeTab === 'applications' ? (
          ...
          <div className="msg-list" style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
            ...
            {applications.map(app => {
              ...
              const statusColors: Record<string,{bg:string;border:string;color:string}> = {
                new:       { bg: 'rgba(139,154,255,.08)', border: 'rgba(139,154,255,.35)', color: '#c8d0ff' },
              ...
              const roleBadge = isAcademy ? { bg: 'rgba(168,107,255,.12)', border: 'rgba(168,107,255,.40)', color: '#d4b8ff', label: 'ACADEMY' } : isBarber ? { bg: 'rgba(139,154,255,.12)', border: 'rgba(139,154,255,.40)', color: '#c8d0ff', label: 'BARBER' } : { bg: 'rgba(143,240,177,.12)', border: 'rgba(143,240,177,.40)', color: '#c9ffe1', label: 'ADMIN' }
              return (
            ...
            <div style={{ padding: '4px 10px', paddingBottom: 'max(6px, env(safe-area-inset-bottom))', flexShrink: 0, display: 'flex', gap: 6, alignItems: 'center', background: 'rgba(18,18,18,.50)', backdropFilter: 'blur(30px) saturate(180%)', WebkitBackdropFilter: 'blur(30px) saturate(180%)', borderTop: '1px solid rgba(255,255,255,.04)' } as React.CSSProperties}>
              {isRecording && voiceSupported ? (
                ...
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, height: 36 }}>
                  ...
                  <button onClick={handleVoiceToggle} style={{ width: 34, height: 34, borderRadius: 999, border: 'none', background: 'linear-gradient(135deg, rgba(139,154,255,.85), rgba(10,100,220,.90))', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 12px rgba(139,154,255,.30)' }}>
                ...
                <>
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <button onClick={() => setShowAttachMenu(v => !v)}
                      style={{ width: 34, height: 34, borderRadius: 999, border: `1px solid ${showAttachMenu ? 'rgba(139,154,255,.40)' : 'rgba(255,255,255,.08)'}`, background: showAttachMenu ? 'rgba(139,154,255,.10)' : 'rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all .2s ease', transform: showAttachMenu ? 'rotate(45deg)' : 'none', color: showAttachMenu ? '#c8d0ff' : 'rgba(255,255,255,.40)' }}>
                      ...
                      <>
                        ...
                        <div style={{ position: 'absolute', bottom: '100%', left: 0, marginBottom: 8, zIndex: 99, display: 'flex', flexDirection: 'column', gap: 4, padding: '6px', borderRadius: 16, background: 'rgba(20,20,20,.92)', border: '1px solid rgba(255,255,255,.12)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', boxShadow: '0 8px 32px rgba(0,0,0,.5)', animation: 'reactionPopIn .2s ease', minWidth: 150 }}>
                          <label onClick={() => setShowAttachMenu(false)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, cursor: 'pointer', background: 'transparent', border: 'none', color: '#e9e9e9', fontSize: 13, fontWeight: 600, fontFamily: 'inherit' }}
                            ...
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="rgba(139,154,255,.80)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
                            Photo
                  ...
                  {hasContent ? (
                    ...
                    <button onClick={sendMessage} disabled={sending}
                      ...
                      style={{ width: 34, height: 34, borderRadius: 999, border: 'none', background: 'linear-gradient(135deg, rgba(139,154,255,.85), rgba(10,100,220,.90))', color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all .2s' }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
                    </button>
                  ) : voiceSupported ? (
