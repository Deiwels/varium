      ...
      <main style={{ minHeight: '100vh', maxWidth: 800, margin: '0 auto', padding: 'clamp(100px, 12vh, 140px) 24px 80px', position: 'relative', zIndex: 2 }}>
        ...
        {hasSmsBusinessContext && (
          <div style={{ ...highlight, marginTop: 0 }}>
            <p style={{ ...text, marginBottom: 12 }}>
              <strong style={{ color: 'rgba(255,255,255,.6)' }}>{smsBusinessName}</strong> uses VuriumBook as its online booking and messaging platform. These terms explain the booking and appointment-messaging rules that apply when a client books with {smsBusinessName} through VuriumBook.
            </p>
            <p style={{ ...text, marginBottom: 0 }}>
              Reviewer context: the booking business is <strong style={{ color: 'rgba(255,255,255,.6)' }}>{smsBusinessName}</strong>; Vurium Inc. is the technology provider. {smsBookingPath ? <>Return to the public booking page: <a href={smsBookingPath} style={{ color: 'rgba(130,150,220,.7)', textDecoration: 'none' }}>{smsBookingPath}</a>.</> : null}
            </p>
          </div>
        )}
        ...
        {hasSmsBusinessContext && (
          <div style={{ ...highlight, marginTop: 18 }}>
            ...
            <p style={{ ...text, marginBottom: 0 }}>
              Customers opt in to receive appointment confirmations, reminders, reschedules, and cancellations for {smsBusinessName}; VuriumBook supplies the booking and messaging infrastructure behind that program.
            </p>
          </div>
        )}
        <div style={highlight}>
