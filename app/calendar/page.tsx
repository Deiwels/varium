...
const OFFHOURS_GLASS: React.CSSProperties = { backdropFilter: 'blur(8px) saturate(100%)', WebkitBackdropFilter: 'blur(8px) saturate(100%)' }
const OFFHOURS_BG = 'rgba(255,255,255,.03)'
const OFFHOURS_BORDER = 'rgba(255,255,255,.06)'
const OFFHOURS_TIME_PILL: React.CSSProperties = { fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 999, background: 'rgba(255,255,255,.06)', border: '1px solid rgba(255,255,255,.08)', color: 'rgba(255,255,255,.30)', letterSpacing: '.04em', fontFamily: 'Inter,sans-serif' }
  ...
  const loadBookings = useCallback(async (barbersArg: Barber[], servicesArg: Service[]) => {
    ...
    return list.map((b: any) => {
      ...
      if (svcNameStr.includes(' + ') && svcs.length < svcNameStr.split(' + ').length) {
        const names = svcNameStr.split(' + ').map((n: string) => n.trim().toLowerCase())
        const byName = names.map(n => servicesArg.find(s => s.name.toLowerCase() === n)).filter(Boolean) as typeof servicesArg
        if (byName.length > svcs.length) {
          svcs = byName
          rawServiceIds = byName.map(s => s.id)
        }
      }
      // Fallback: if duration is much longer than found services, try matching by combined service name
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100dvh', background: 'transparent', color: '#e8e8ed', fontFamily: 'Inter,system-ui,sans-serif' }}>
        ...
        {portalTarget && createPortal(
          <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              ...
              {/* Settings */}
              {(isOwnerOrAdmin || hasPerm('calendar_settings', 'open_settings')) && <button onClick={() => setSettingsOpen(true)} style={{ height: 26, width: 26, borderRadius: 7, border: '1px solid rgba(255,255,255,.06)', background: 'rgba(255,255,255,.03)', color: 'rgba(255,255,255,.4)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }} className="cal-settings-icon">
        ...
        {(() => {
          ...
          return (
            ...
            {typeof document !== 'undefined' && !modal.open && createPortal(
            <div className="date-dot-wrap" style={{ position: 'fixed', bottom: typeof window !== 'undefined' && (window as any).__VURIUM_IS_NATIVE ? 74 : 50, left: 0, right: 0, zIndex: 10001, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 5, padding: '3px 0', pointerEvents: 'auto' }}>
            ...
            {typeof document !== 'undefined' && !modal.open && createPortal(
            <div style={{ position: 'fixed', bottom: typeof window !== 'undefined' && (window as any).__VURIUM_IS_NATIVE ? 56 : 14, left: 0, right: 0, zIndex: 10003, padding: '0 8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', pointerEvents: 'none' }}>
              <button onClick={() => setSettingsOpen(true)} style={{ width: 40, height: 40, borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.60)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', color: 'rgba(255,255,255,.50)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto', boxShadow: '0 2px 12px rgba(0,0,0,.4)' }}>
              ...
              }} style={{ width: 40, height: 40, borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(0,0,0,.60)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', color: 'rgba(255,255,255,.50)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto', boxShadow: '0 2px 12px rgba(0,0,0,.4)' }}>
