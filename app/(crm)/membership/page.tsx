              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, fontSize: 12, color: 'rgba(255,255,255,.50)' }}>
                    ...
                    <span style={{ color: 'rgba(255,255,255,.20)' }}>·</span>
                    <span style={{ color: '#c8d0ff', fontWeight: 700 }}>{FREQ_LABELS[m.frequency] || m.frequency}</span>
                  </div>
          ...
          <div style={{ width: 'min(440px,100%)', maxHeight: '90vh', overflowY: 'auto', borderRadius: 24, border: '1px solid rgba(255,255,255,.08)', background: 'rgba(6,6,6,.96)', color: '#e9e9e9', fontFamily: 'Inter,sans-serif', boxShadow: '0 30px 80px rgba(0,0,0,.7)', animation: 'memModalIn .3s cubic-bezier(.4,0,.2,1)', WebkitOverflowScrolling: 'touch' as any }}>
            ...
            <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {!editing && <>
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    }} style={{ height: 44, padding: '0 14px', borderRadius: 12, border: '1px solid rgba(139,154,255,.40)', background: 'rgba(139,154,255,.08)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 700, fontSize: 12, fontFamily: 'inherit', whiteSpace: 'nowrap' }}>Search</button>
                  </div>
                  ...
                  {services.filter(s => !fBarber || !s.barberIds.length || s.barberIds.includes(fBarber)).map(s => {
                    ...
                    return (
                      <button key={s.id} onClick={() => setFServiceIds(prev => on ? prev.filter(x => x !== s.id) : [...prev, s.id])}
                        style={{ height: 36, padding: '0 12px', borderRadius: 10, border: `1px solid ${on ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.10)'}`, background: on ? 'rgba(139,154,255,.15)' : 'rgba(255,255,255,.03)', color: on ? '#c8d0ff' : 'rgba(255,255,255,.50)', cursor: 'pointer', fontSize: 12, fontWeight: on ? 800 : 500, fontFamily: 'inherit', transition: 'all .2s', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  {(['weekly', 'biweekly', 'monthly'] as const).map(f => (
                    <button key={f} onClick={() => setFFreq(f)}
                      style={{ flex: 1, height: 38, borderRadius: 10, border: `1px solid ${fFreq === f ? 'rgba(139,154,255,.50)' : 'rgba(255,255,255,.10)'}`, background: fFreq === f ? 'rgba(139,154,255,.12)' : 'rgba(255,255,255,.03)', color: fFreq === f ? '#c8d0ff' : 'rgba(255,255,255,.45)', cursor: 'pointer', fontWeight: 700, fontSize: 11, fontFamily: 'inherit', transition: 'all .2s' }}>
                      {FREQ_LABELS[f]}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                ...
                <button onClick={handleSave} disabled={saving} style={{ flex: 1, height: 44, borderRadius: 12, border: '1px solid rgba(139,154,255,.50)', background: 'rgba(139,154,255,.12)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 13, fontFamily: 'inherit', opacity: saving ? .5 : 1 }}>
