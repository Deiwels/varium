    ...
    <div style={glassModal} onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={modalBox}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 18px', borderBottom: '1px solid rgba(255,255,255,.07)' }}>
          <div style={{ letterSpacing: '.16em', textTransform: 'uppercase', fontSize: 13, fontWeight: 700 }}>My Profile</div>
        ...
        <div style={{ padding: '16px 18px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {tab === 'notifications' && <>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,.50)', marginBottom: 4 }}>Choose which push notifications you receive in the VuriumBook app.</div>
            {([
              ...
              <div key={item.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
                ...
                <button onClick={() => setNotifPrefs(prev => ({ ...prev, [item.key]: !prev[item.key] }))}
                  style={{ width: 44, height: 26, borderRadius: 999, border: 'none', cursor: 'pointer', padding: 2, transition: 'background .2s', background: notifPrefs[item.key] ? 'rgba(139,154,255,.35)' : 'rgba(255,255,255,.10)', position: 'relative', flexShrink: 0 }}>
                  <div style={{ width: 22, height: 22, borderRadius: 999, background: notifPrefs[item.key] ? '#8b9aff' : 'rgba(255,255,255,.30)', transition: 'transform .2s, background .2s', transform: notifPrefs[item.key] ? 'translateX(18px)' : 'translateX(0)' }} />
                </button>
              </div>
            ))}
          ...
          {msg && <div style={{ fontSize: 12, color: '#c8d0ff', padding: '8px 12px', borderRadius: 10, border: '1px solid rgba(139,154,255,.22)', background: 'rgba(139,154,255,.06)' }}>{msg}</div>}
...
export default function Shell({ children, page }: { children: React.ReactNode; page: string }) {
  ...
  useEffect(() => {
    ...
    const CHAT_COLORS: Record<string, string> = { general: '#c8d0ff', barbers: '#c8d0ff', admins: '#c9ffe1', students: '#d4b8ff', requests: '#ffe9a3', applications: '#ffb7d5' }
    const chatTypes = ['general', 'barbers', 'admins', 'students']
    const lastSeenKey = 'VURIUMBOOK_MSG_LAST_SEEN'
    const lastSeenAppsKey = 'VURIUMBOOK_APPS_LAST_SEEN'
    const lastSeenReqKey = 'VURIUMBOOK_REQ_LAST_SEEN'
      ...
      try {
        ...
        for (const ct of chatTypes) {
          ...
          if (msgs.length && msgs[msgs.length - 1]?.createdAt > lastSeen && msgs[msgs.length - 1]?.senderId !== user.uid) {
            setUnreadChat(CHAT_COLORS[ct] || '#c8d0ff')
            return
          }
        }
  ...
  useEffect(() => {
    if (pathname === '/messages') {
      ...
      localStorage.setItem('VURIUMBOOK_MSG_LAST_SEEN', now)
      localStorage.setItem('VURIUMBOOK_APPS_LAST_SEEN', now)
      localStorage.setItem('VURIUMBOOK_REQ_LAST_SEEN', now)
    }
  }, [pathname])
      ...
      {showPinOverlay && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,.95)', backdropFilter: 'blur(20px)', padding: 20, fontFamily: 'Inter, system-ui, sans-serif' }}>
          <div style={{ width: '100%', maxWidth: 360, textAlign: 'center' }}>
            <div style={{ letterSpacing: '.22em', textTransform: 'uppercase', fontSize: 18, marginBottom: 6, color: '#e9e9e9', fontWeight: 700 }}>VuriumBook</div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,.40)', marginBottom: 8 }}>Session expired</div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,.55)', marginBottom: 28 }}>Enter your PIN to continue as <strong style={{ color: '#c8d0ff' }}>{user?.name || getPinUsername()}</strong></div>
            {/* PIN dots */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: 14, marginBottom: 24 }}>
              {[0, 1, 2, 3].map(i => (
                <div key={i} style={{ width: 16, height: 16, borderRadius: '50%', border: '2px solid rgba(255,255,255,.20)', background: i < pinInput.length ? '#8b9aff' : 'transparent', transition: 'background .15s' }} />
              ))}
            </div>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .brand h1{
          ...
          font-weight:800;
          letter-spacing:.22em;font-size:14px;text-transform:uppercase;
          background:linear-gradient(150deg,rgba(139,154,255,.92),rgba(139,154,255,.45));
        ...
        .nav-item.active{
          background:rgba(139,154,255,.09);
          border-color:rgba(139,154,255,.12);
        }
        .nav-ico{
        ...
        .nav-item.active .nav-ico{
          background:rgba(139,154,255,.12);
          border-color:rgba(139,154,255,.14);
        }
        ...
        .swipe-hint {
          ...
          background:rgba(139,154,255,.30);
          z-index:60;
        ...
        <aside className={`sidebar${sidebarOpen ? ' open' : ''}`}>
              ...
              >
                {(user as any)?.photo
                  ? <img
                    ...
                    />
                  : <div style={{ width: 34, height: 34, borderRadius: 9, background: 'rgba(139,154,255,.10)', border: '1px solid rgba(139,154,255,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 900, color: '#fff', flexShrink: 0 }}>
          ...
          <nav className="nav">
            {visibleNav.map(item => {
              ...
              const pc = unreadChat || '#c8d0ff'
              const r = parseInt(pc.slice(1,3),16)||200, g = parseInt(pc.slice(3,5),16)||208, b = parseInt(pc.slice(5,7),16)||255
              return (
                <Link
                  key={item.id}
                  href={item.href}
