  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      <button onClick={() => { setOpen(!open); const dd = value ? new Date(value + 'T12:00:00') : new Date(); setViewYear(dd.getFullYear()); setViewMonth(dd.getMonth()) }}
        style={{ width: '100%', height: 44, borderRadius: 14, border: `1px solid ${open ? 'rgba(139,154,255,.50)' : 'rgba(255,255,255,.12)'}`, background: open ? 'rgba(139,154,255,.06)' : 'rgba(0,0,0,.22)', color: '#fff', padding: '0 14px', fontSize: 14, fontWeight: 600, fontFamily: 'inherit', cursor: 'pointer', textAlign: 'left', transition: 'all .2s ease', boxShadow: open ? '0 0 12px rgba(139,154,255,.15)' : 'none' }}>
        {displayDate}
      </button>
      {open && (
            ...
            {cells.map((day, i) => {
              ...
              return (
                <button key={i} onClick={() => { onChange(dateStr); setOpen(false) }}
                  style={{ width: '100%', height: 36, borderRadius: 10, border: isSelected ? '1px solid rgba(139,154,255,.65)' : isToday ? '1px solid rgba(255,255,255,.25)' : '1px solid transparent', background: isSelected ? 'rgba(139,154,255,.20)' : 'transparent', color: isSelected ? '#fff' : isToday ? '#c8d0ff' : 'rgba(255,255,255,.75)', cursor: 'pointer', fontWeight: isSelected || isToday ? 900 : 500, fontSize: 13, fontFamily: 'inherit', transition: 'all .15s ease', boxShadow: isSelected ? '0 0 14px rgba(139,154,255,.25)' : 'none' }}>
                  {day}
                </button>
              )
            })}
  ...
  return (
    <Shell page="cash">
      <style>{`
        ...
        @keyframes kpiPulse { 0%{box-shadow:0 0 0 0 rgba(139,154,255,0)} 50%{box-shadow:0 0 16px 0 rgba(139,154,255,.12)} 100%{box-shadow:0 0 0 0 rgba(139,154,255,0)} }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#000', color: '#e9e9e9', fontFamily: 'Inter,system-ui,sans-serif' }}>
        ...
        <div style={{ padding: '16px 20px 14px', background: 'linear-gradient(to bottom, rgba(0,0,0,.90), rgba(0,0,0,.70))', backdropFilter: 'blur(18px) saturate(160%)', WebkitBackdropFilter: 'blur(18px) saturate(160%)', borderBottom: '1px solid rgba(255,255,255,.06)', position: 'sticky', top: 0, zIndex: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            {loading && <div style={{ width: 18, height: 18, borderRadius: 999, border: '2px solid rgba(255,255,255,.15)', borderTopColor: 'rgba(139,154,255,.80)', animation: 'spin .7s linear infinite' }} />}
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 12 }}>
            {[{ id: 'today', label: 'Today' }, { id: 'week', label: '7 days' }, { id: 'month', label: '30 days' }].map(p => (
              <button key={p.id} className="cash-preset" onClick={() => setPreset(p.id)}
                style={{ height: 32, padding: '0 14px', borderRadius: 999, border: `1px solid ${activePreset === p.id ? 'rgba(139,154,255,.55)' : 'rgba(255,255,255,.10)'}`, background: activePreset === p.id ? 'rgba(139,154,255,.14)' : 'rgba(255,255,255,.03)', color: activePreset === p.id ? '#c8d0ff' : 'rgba(255,255,255,.55)', cursor: 'pointer', fontWeight: 800, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.08em', fontFamily: 'inherit', boxShadow: activePreset === p.id ? '0 0 10px rgba(139,154,255,.15)' : 'none' }}>
                {p.label}
              </button>
            ))}
          </div>
        ...
        <div className="cash-kpi-grid cash-page-pad" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, padding: '14px 20px' }}>
          {[
            ...
            { label: 'Zelle', value: usd(totalExpectedZelle), color: '#c8d0ff', border: 'rgba(139,154,255,.18)', bg: 'rgba(139,154,255,.04)', delay: '.05s', glow: 'rgba(139,154,255,.12)', perfectGlow: false },
        ...
        <div className="cash-page-pad" style={{ flex: 1, overflowY: 'auto', padding: '0 20px 100px' }}>
          {loading && days.length === 0 ? (
            <div style={{ padding: 50, textAlign: 'center' }}>
              <div style={{ width: 28, height: 28, borderRadius: 999, border: '2px solid rgba(255,255,255,.12)', borderTopColor: 'rgba(139,154,255,.70)', animation: 'spin .7s linear infinite', margin: '0 auto 12px' }} />
            ...
            return (
              <div key={day.date} className="cash-day cash-card" style={{
                ...
                border: `1px solid ${isToday ? 'rgba(139,154,255,.25)' : 'rgba(255,255,255,.07)'}`,
                background: isToday ? 'rgba(139,154,255,.03)' : 'rgba(255,255,255,.015)',
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: hasActivity ? 10 : 0 }}>
                  <div>
                    <div style={{ fontWeight: 900, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
                      {dayLabel}
                      {isToday && <span style={{ fontSize: 9, padding: '2px 8px', borderRadius: 999, border: '1px solid rgba(139,154,255,.35)', background: 'rgba(139,154,255,.10)', color: '#c8d0ff', fontWeight: 800, letterSpacing: '.06em' }}>TODAY</span>}
                ...
                {hasActivity && (
                  <div className="cash-day-nums" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
                    ...
                    <div style={{ padding: '10px 10px', borderRadius: 12, background: 'rgba(139,154,255,.04)', border: '1px solid rgba(139,154,255,.10)' }}>
                      <div style={{ fontSize: 8, color: 'rgba(139,154,255,.55)', letterSpacing: '.10em', textTransform: 'uppercase', fontWeight: 800 }}>Zelle</div>
                      <div style={{ fontSize: 17, fontWeight: 900, color: '#c8d0ff', marginTop: 3 }}>{usd(expectedZelle)}</div>
                ...
                {/* Edit form */}
                {isEditing && (
                  <div style={{ marginTop: 10, padding: '12px 12px', borderRadius: 14, border: '1px solid rgba(139,154,255,.20)', background: 'rgba(139,154,255,.03)', animation: 'cashSlide .25s ease' }}>
                    <div className="cash-edit-row" style={{ display: 'flex', gap: 8, alignItems: 'flex-end' }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <input type="number" min="0" step="0.01" value={cashInput} onChange={e => setCashInput(e.target.value)} placeholder="0.00" autoFocus
                          style={{ width: '100%', height: 44, borderRadius: 12, border: '1px solid rgba(139,154,255,.30)', background: 'rgba(0,0,0,.25)', color: '#fff', padding: '0 12px', outline: 'none', fontSize: 16, fontWeight: 700, fontFamily: 'inherit' }} />
                      </div>
                      <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                      ...
                      <button onClick={() => saveReport(day.date)} disabled={saving === day.date}
                        style={{ flex: 2, height: 38, borderRadius: 12, border: '1px solid rgba(139,154,255,.55)', background: 'rgba(139,154,255,.10)', color: '#c8d0ff', cursor: 'pointer', fontWeight: 900, fontSize: 12, fontFamily: 'inherit', boxShadow: '0 0 12px rgba(139,154,255,.15)' }}>
