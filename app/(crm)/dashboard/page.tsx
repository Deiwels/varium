...
const STATUS_STYLE: Record<string, React.CSSProperties> = {
  ...
  booked:    { borderColor: 'rgba(139,154,255,.40)',  background: 'rgba(139,154,255,.10)',  color: '#c8d0ff' },
...
function KpiCard({ title, value, sub, color }: { title: string; value: string; sub: string; color?: string }) {
  const dots: Record<string, string> = { ok: '#8ff0b1', bad: '#ff6b6b', blue: '#8b9aff', gold: '#ffcf3f' }
  return (
...
export default function DashboardPage() {
  ...
  const [clockOutSummary, setClockOutSummary] = useState<{ hours: string; earnings: string; tips: string; clients: number; services: string } | null>(() => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem('VURIUMBOOK_CLOCKOUT_SUMMARY') || 'null')
    ...
    try {
      ...
      if (wasClocked) {
        ...
        try {
          ...
          try { localStorage.setItem('VURIUMBOOK_CLOCKOUT_SUMMARY', JSON.stringify({ date: isoToday(), data: summaryData })) } catch {}
        } catch {
          const totalMins = data?.duration_minutes || todayMinutes
          const h = Math.floor(totalMins / 60)
          const m = totalMins % 60
          ...
          try { localStorage.setItem('VURIUMBOOK_CLOCKOUT_SUMMARY', JSON.stringify({ date: isoToday(), data: fallbackData })) } catch {}
        }
        setClockSuccess('out')
          ...
          <div className={clockSuccess === 'out' ? 'clock-out-success-card' : ''} style={{ borderRadius: 18, border: '1px solid rgba(143,240,177,.20)', background: 'linear-gradient(180deg,rgba(143,240,177,.06),rgba(0,0,0,.30))', boxShadow: '0 10px 40px rgba(0,0,0,.35)', padding: '18px 16px', marginBottom: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              ...
              <div className="clock-summary-item" style={{ padding: '10px 12px', borderRadius: 12, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 900, color: '#c8d0ff' }}>{clockOutSummary.services}</div>
              </div>
              <div className="clock-summary-item" style={{ padding: '10px 12px', borderRadius: 12, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)' }}>
        ...
        {isBarber ? (
              <div style={{ borderRadius: 18, border: '1px solid rgba(255,255,255,.10)', background: 'linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.02))', padding: 14, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    {(['today', 'week', 'month'] as EarningsPeriod[]).map(p => (
                      <button key={p} onClick={() => { setEarningsPeriod(p); setEarningsOffset(0) }} style={{
                        height: 24, padding: '0 8px', borderRadius: 6, border: `1px solid ${earningsPeriod === p ? 'rgba(139,154,255,.45)' : 'rgba(255,255,255,.10)'}`,
                        background: earningsPeriod === p ? 'rgba(139,154,255,.12)' : 'transparent',
                        color: earningsPeriod === p ? '#c8d0ff' : 'rgba(255,255,255,.35)', fontSize: 9, fontWeight: 700,
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, marginBottom: 10 }}>
                  ...
                  <div style={{ fontSize: 12, color: earningsOffset === 0 ? 'rgba(255,255,255,.60)' : '#c8d0ff', fontWeight: 600, minWidth: 120, textAlign: 'center' }}>
                ...
                {loading && !myPayroll ? <div style={{ color: 'rgba(255,255,255,.35)', fontSize: 12 }}>Loading…</div> : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {[
                      { label: 'Services', value: money(myPayroll?.barber_service_share || 0), color: '#c8d0ff' },
                      { label: 'Tips', value: money(barberTips), color: '#8ff0b1' },
                      { label: 'Total payout', value: money(barberEarnings), color: '#fff', big: true },
                    ].map(row => (
                      <div key={row.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', borderRadius: 12, border: '1px solid rgba(255,255,255,.08)', background: row.big ? 'rgba(139,154,255,.08)' : 'rgba(0,0,0,.14)', borderColor: row.big ? 'rgba(139,154,255,.30)' : 'rgba(255,255,255,.08)' }}>
                ...
                {Object.entries(byBarber).sort((a, b) => b[1] - a[1]).map(([name, count]) => (
                  <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: '1px solid rgba(255,255,255,.05)' }}>
                    ...
                    <div style={{ flex: 1, height: 6, borderRadius: 999, background: 'rgba(255,255,255,.08)' }}>
                      <div style={{ height: 6, borderRadius: 999, background: 'linear-gradient(90deg,#8b9aff,rgba(139,154,255,.5))', width: `${Math.round(count / maxCount * 100)}%`, transition: 'width .6s ease' }} />
                    </div>
                ...
                {barbers.map((b: any) => {
                  ...
                  return (
                    <button key={b.id} onClick={() => setReviewFilter(b.id)}
                      style={{ height: 28, padding: '0 10px', borderRadius: 999, border: `1px solid ${reviewFilter === b.id ? 'rgba(139,154,255,.45)' : 'rgba(255,255,255,.08)'}`, background: reviewFilter === b.id ? 'rgba(139,154,255,.10)' : 'transparent', color: reviewFilter === b.id ? '#c8d0ff' : 'rgba(255,255,255,.55)', cursor: 'pointer', fontSize: 11, fontWeight: 700, fontFamily: 'inherit' }}>
                      {b.name} ({cnt})
                    </button>
                  )
                })}
                      ...
                      {pending.map((r: any) => (
                        <div key={r.id} style={{ padding: '12px 14px', borderRadius: 14, border: '1px solid rgba(255,207,63,.25)', background: 'rgba(255,207,63,.04)' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
                            ...
                            {r.barber_name && <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 999, border: '1px solid rgba(139,154,255,.25)', background: 'rgba(139,154,255,.06)', color: 'rgba(139,154,255,.80)', letterSpacing: '.06em', textTransform: 'uppercase' }}>{r.barber_name}</span>}
                          </div>
